...
func _set_route_and_target():
	...
	for t in targets:
		...
		while !routes.is_empty() and loops < max_loops:
			...
			if (t.global_position - _get_global_pos(current.pos)).length() <= get_range():
				end_pos = current.pos
				found = true
				break
			for nb in tile_neighbors:
				var nb_pos = current.pos + nb
				if !_has_tile_at_pos(nb_pos) or !tile_map.tile_buildings.has(nb_pos): continue
				if nb_pos != end_pos and tile_map.tile_buildings[nb_pos] != null: continue
